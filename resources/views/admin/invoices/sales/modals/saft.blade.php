<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal">
        <span class="fs-15" aria-hidden="true"><i class="fas fa-times"></i></span>
        <span class="sr-only">Fechar</span>
    </button>
    <h4 class="modal-title">Consulta de ficheiros SAF-T</h4>
</div>
<div class="modal-body">
    <div style="background: #e9e9e9;
    border-bottom: 1px solid #ddd;
    margin: -15px -15px 15px -15px;
    padding: 15px;">
        Esta ferramenta permite-lhe a <b>emissão e download do ficheiro SAF-T</b> ou o
        seu <b>envio automáticamente para a sua contabilidade</b> em qualquer altura.
    </div>
    @if(hasModule('invoices'))
        <div class="form-group m-0">
            {{ Form::select('company_id', $companies, $companyId, ['class' => 'form-control select2']) }}
        </div>
    <div class="table-responsive">
        <table id="datatable-saft" class="table table-condensed table-striped table-dashed table-hover">
            <thead>
                <tr>
                    <th class="w-120px">Mês</th>
                    <th>Gerado por</th>
                    <th class="w-110px">Criado em</th>
                    <th class="w-1">SAF-T</th>
                </tr>
            </thead>
            <tbody>
            </tbody>
        </table>
    </div>
    @else
        <div class="h-30"></div>
        <div class="row row-5">
            <div class="col-sm-3">
                <img class="img-responsive pull-right p-10" src="data:image/svg+xml;utf8;base64," />
            </div>
            <div class="col-sm-9">

                <h3 class="m-t-0 m-b-5">Módulo não incluído</h3>
                <h4>A sua licença não inclui software de faturação.</h4>
                <p class="text-muted">
                    Tenha a possibilidade de emitir faturas automáticamente,
                    contas correntes entre outras funcionalidades à distância de um clique.
                </p>
                <a href="mailto:info@quickbox.pt?subject=Pedido de informação sobre módulo&body=Pretendia conhecer mais detalhes e valor para o módulo de Faturação."
                   class="btn btn-sm btn-primary m-t-10">
                    <i class="fas fa-envelope"></i> Pedir informações
                </a>
            </div>
        </div>
    @endif
</div>
<div class="modal-footer">
    <div class="pull-right">
        <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
    </div>
</div>
{{ Form::close() }}
<style>
    .modal-dialog.modal-xs {
        margin-top: 180px;
    }
</style>

<script>
    $('.modal .select2').select2(Init.select2());

    $('.modal [name="company_id"]').on('change', function(){
        var id = $(this).val();

        $.get("{{ route('admin.invoices.saft') }}", {'company':id}, function(data){
            $('#modal-remote .modal-content').html(data)
        })
    })

    var oTableSaft = $('#datatable-saft').DataTable({
        dom: "<'row row-0'>" +
            "<'row row-0'<'col-sm-12'tr>>" +
            "<'row row-0'<'col-sm-5'l><'col-sm-7'p>>",
        serverSide: false,
        pageLength: 10,
        order: [[0, "desc"]],
        "data" : {!! json_encode(@$safts) !!},
        "columns" : [
            /*{data: 'id', visible: false},*/
            {data: "id" },
            {data: "created_by" },
            {data: "created_at" },
            {data: "download" },
        ]
    });
</script>