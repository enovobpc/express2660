...
if(!$shipment->exists) {
    $shipment->sender_vat        = @$customer->agency->vat;
    $shipment->sender_name       = @$customer->agency->company;
    $shipment->sender_address    = @$customer->agency->address;
    $shipment->sender_zip_code   = @$customer->agency->zip_code;
    $shipment->sender_city       = @$customer->agency->city;
    $shipment->sender_agency_id  = $customer->agency_id;

    $shipment->recipient_vat       = $customer->vat;
...
<div class="modal-body p-l-15 p-t-15 p-r-15 p-b-10 modal-shipment" id="modal-{{ $hashId }}">
    ...
    <div class="row">
        @if(!empty($customer->shipments_daily_limit_hour) && $exceeded)
            <div class="col-xs-12">
    ...
    <div class="row row-10">
        <div class="col-xs-12 col-sm-6">
            <div class="row">
                <div class="col-xs-12 col-sm-4" style="width: 37%;margin-right: 15px; padding: 0">
                    <div class="form-group m-b-5">
                    ...
                    <div class="form-group">
                        ...
                        <div class="weight-col {{ !empty($shipment->volumetric_weight) ? 'col-sm-5' : 'col-sm-8' }}">
                            <div class="input-group">
                                {{ Form::text('weight', null, ['class' => 'form-control decimal trigger-price', 'maxlength' => 7, 'data-toggle' => 'tooltip', 'title' => trans('account/shipments.modal-shipment.tips.weight'), 'autocomplete' => 'off']) }}
...
<script>
    ...
    $('.opt-flds-grp .tdinpt input').on('change', function(e) {

        var total = 0;

        $('.opt-flds-grp .tdinpt input').each(function(){
...
console.log(value);
        ...
        $('[name="volumes"]').val(total)
    })

    var CURRENT_DATE = "2023-10-16"; //"{{ date('Y-m-d') }}";
