<?php
$currencySymbol = @$regularization->currency ? @$regularization->currency : Setting::get('app_currency');
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="m-t-60 m-b-60 text-center">
                    ...
                    <p>Só pode emitir regularizações para faturas do mesmo cliente.</p>
                </div>
            </div>
        </div>
    @else
        <div class="row">
            @if (@$customersSameVat && @$customersSameVat->count() > 1)
                <div class="col-sm-12">
                    <div class="alert bg-blue">
                        ...
                        {{ $customer->vat }}.</b>
                        <br />
                        Selecione na caixa de seleção do cliente, a filial/conta-corrente na qual pretende associar a
                        regularização.
                    </div>
            ...
            <div class="col-sm-4">
                <div class="row row-5">
                    <div class="col-sm-12">
                        <h4 class="text-blue m-t-0 m-b-10">Regularização de Conta Corrente</h4>
                    ...
                    <div class="col-sm-5">
                        <div class="form-group is-required">
                            ...
                            <div class="input-group input-group-money">
                                {{ Form::text('docdate', @$regularization->exists ? @$regularization->doc_date : date('Y-m-d'), ['class' => 'form-control','required']) }}
                    ...
                    <div class="col-sm-7">
                        <div class="form-group">
                            ...
                            {{ Form::text('docref', @$regularization->reference, ['class' => 'form-control']) }}
                        </div>
                    </div>
                    {{-- <div class="col-sm-7">
                        <div class="form-group is-required">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            {{ Form::label('payment_method', 'Forma Liquidação') }}
                            {{ Form::select('payment_method', ['' => ''] + $paymentMethods, @$regularization->payment_method, ['class' => 'form-control select2', 'required']) }}
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            {{ Form::select('payment_bank_id', [''=>''] + $banks, @$regularization->bank_id, ['class' => 'form-control select2']) }}
                        </div>
                    </div>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group m-0">
                            ...
                            {{ Form::text('obs', @$regularization->obs, ['class' => 'form-control']) }}
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-8">
                <div class="invoices-list">
                    <h4 class="text-blue m-t-0 m-b-10">Documentos a regularizar</h4>
                    <div class="items-list">
                        @include('admin.invoices.sales.partials.regularization_invoices_list')
                    </div>
                </div>
            </div>
        </div>
...
<div class="modal-footer">
    {{-- REGULARIZAÇÃO NÃO DEVE ENVIAR E-MAIL. MAS FICA JA COMENTADO CASO NECESSÁRIO --}}
    {{-- @if (!@$blocked)
            ...
            <ul class="list-inline pull-left m-t-5 m-b-0">
                <li>
                    <div class="checkbox m-b-0 m-t-5" style="margin-right: -15px">
                        <label>
                            {{ Form::checkbox('attachments[]', 'regularization', @$customer->billing_email ? true : false) }}
                            Regularização
                        </label>
                    </div>
                </li>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">
                    Confirmar emissão de regularização
                </h4>
            </div>
            <div class="modal-body">
                <h4 class="m-t-0">Confirma a emissão da regularização de conta corrente?</h4>
                <p class="m-b-4">Cliente: <span class="ft-nm bold">{{ @$customer->billing_name }}</span>
                </p>
                <p class="m-b-4">Data regularização: <span
                        class="ft-dt bold">{{ @$regularization->exists ? $regularization->docdate : date('Y-m-d') }}</span></p>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    @if (count($customersList) == 1)
        ...
        $.post('{{ route('admin.invoices.sales.get.customer.invoices') }}', {customerId: customerId, 'target': 'regularization'}, function (data) {
    ...
    $('.form-billing').on('submit', function(e) {
        ...
        if ($('.form-billing [name="customer_id"]').val() == '') {
            ...
            Growl.error('Não pode emitir regularizações com valor negativo.')
