{{ Form::open(['route' => ['admin.billing.customers.mass.billing.store', 'month' => $month, 'year' => $year, 'period' => $period], 'class' => 'mass-billing-form']) }}
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal">
        <span class="fs-15" aria-hidden="true"><i class="fas fa-times"></i></span>
        <span class="sr-only">Fechar</span>
    </button>
    <h4 class="modal-title">Emitir faturação em massa</h4>
</div>
<div class="modal-body">
    @if(hasModule('invoices-advanced'))
    @if($period == '30d')
        <h4>Pretende emitir todas as faturas do mês <b>{{ trans('datetime.month.' . $month) }} {{ $year }}</b>?</h4>
    @elseif($period == '1q')
        <h4>Pretende emitir todas as faturas da 1ª quinzena de {{ trans('datetime.month.' . $month) }} {{ $year }} automáticamente?</h4>
    @elseif($period == '2q')
        <h4>Pretende emitir todas as faturas da 2ª quinzena de {{ trans('datetime.month.' . $month) }} {{ $year }} automáticamente?</h4>
    @endif
    <p class="text-blue m-t-10 m-b-0">
        <ul class="list-inline text-blue">
            <li>
                <i class="fas fa-info-circle"></i> Este processo vai demorar alguns minutos.
            </li>
            <li>
                <i class="fas fa-info-circle"></i> Caso existam clientes com anomalias na faturação (ex: serviços sem preço ou outros) estes não serão faturados.
            </li>
        </ul>
    </p>
    @else
        <div class="row">
            <div class="col-sm-3">
                <img class="w-120px pull-right" src="data:image/svg+xml;utf8;base64," />
            </div>
            <div class="col-sm-9">
                <h3 class="m-t-0">Módulo não incluído</h3>
                <h4>O seu plano contratado não inclui esta funcionalidade.</h4>
                <p class="text-muted">
                    <i class="fas fa-check"></i> Emissão automática de todas as faturas<br/>
                    <i class="fas fa-check"></i> Emissão automática de faturas-proforma + Ref. Multibanco<br/>
                    <i class="fas fa-check"></i> Emissão automática de faturas após bom pagamento<br/>
                    <i class="fas fa-check"></i> Emissão de documentos internos<br/>
                    <i class="fas fa-check"></i> Criação de faturas agendadas
                </p>
                <a href="mailto:info@quickbox.pt?subject=Pedido de informação sobre módulo&body=Pretendia conhecer mais detalhes e valor para o módulo de faturação avançada"
                   class="btn btn-sm btn-primary m-t-10">
                    <i class="fas fa-envelope"></i> Pedir informações
                </a>
            </div>
        </div>
    @endif
    <hr/>
    <div class="row">
        <div class="col-sm-4">
            <div class="form-group">
                {{ Form::label('api_key', 'Série') }}
                {{ Form::select('api_key', $apiKeys, null, ['class' => 'form-control select2']) }}
            </div>
            <div class="form-group">
                {{ Form::label('docdate', 'Data Documento') }}
                <div class="input-group">
                    {{ Form::text('docdate', $docDate, ['class' => 'form-control datepicker']) }}
                    <div class="input-group-addon">
                        <i class="fas fa-calendar-alt"></i>
                    </div>
                </div>
            </div>
            <div class="form-group">
                {{ Form::label('reference', 'Referência') }}
                {{ Form::text('reference', $reference, ['class' => 'form-control']) }}
            </div>
            <div class="form-group">
                {{ Form::label('payment_condition', 'Condições de Pagamento') }}
                {{ Form::select('payment_condition', $paymentConditions, null, ['class' => 'form-control select2']) }}
            </div>
            <ul class="list-unstyled">
                <li>
                    <div class="checkbox m-b-0 m-t-5" style="margin-right: -15px">
                        <label style="padding: 0">
                            {{ Form::checkbox('send_email', 1, true) }}
                            Enviar e-mail ao cliente
                        </label>
                    </div>
                </li>
                <li>
                    <div class="checkbox m-b-0 m-t-5" style="margin-right: -15px">
                        <label style="padding: 0">
                            {{ Form::checkbox('attachments[]', 'summary', true) }}
                            Anexar Resumo PDF
                        </label>
                    </div>
                </li>
                <li>
                    <div class="checkbox m-b-0 m-t-5" style="margin-right: -15px">
                        <label style="padding: 0">
                            {{ Form::checkbox('attachments[]', 'excel', Setting::get('billing_attach_excel')) }}
                            Anexar Resumo Excel
                        </label>
                    </div>
                </li>
                <li>
                    <div class="checkbox m-b-0 m-t-5" style="margin-right: -15px">
                        <label style="padding: 0">
                            {{ Form::checkbox('attachments[]', 'invoice', true) }}
                            Anexar Fatura
                        </label>
                    </div>
                </li>
            </ul>
        </div>
        <div class="col-sm-8">
            {{ Form::label('customer_id', 'Clientes a faturar') }}
            <div style="height: 350px; overflow-y: auto; width: 100%; border: 1px solid #ccc; border-radius: 3px" class="customers-zone">
                <div>
                    {{ Form::text('search', null, ['class' => 'form-control', 'placeholder' => 'Procurar...']) }}
                </div>
                <div style="height: 314px; overflow-y: auto; border-radius: 3px;">
                    <table class="table table-condensed">
                        <tr>
                            <th class="bg-gray w-1">{{ Form::checkbox('customer_all[]', 1, true, ['class' => 'row-all', 'style' => 'border-left: none; border-right: none; border-top:none']) }}</th>
                            <th class="bg-gray">Cliente</th>
                            <th class="bg-gray w-1 text-right">Subtotal</th>
                        </tr>
                        <?php $currency = Setting::get('app_currency'); $countEmptyVat = 0; ?>
                        @foreach($customers as $customer)
                            <?php
                            $price = 0;

                            if(empty($customer->vat) || $customer->vat == '999999990') {
                                $countEmptyVat++;
                            }

                            if(!empty($customer->shipments)) {
                                $price+= (@$customer->shipments->sum('total_price') + @$customer->shipments->sum('total_expenses')  + @$customer->shipments->sum('fuel_price') + @$customer->shipments->sum('total_price_for_recipient'));
                            }

                            if(!empty($customer->covenants)) {
                                $price+= @$customer->covenants->sum('amount');
                            }

                            if(!empty($customer->productsBought)) {
                                $price+= @$customer->productsBought->sum('subtotal');
                            }

                            ?>
                            <tr style="{{ $customer->ignore_mass_billing ? 'color: #ccc' : '' }}"  data-toggle="{{ $customer->ignore_mass_billing ? 'tooltip' : '' }}" title="{{ $customer->ignore_mass_billing ? "Ignorado da faturação massiva" : '' }}" data-filter-text="{{ strtolower(removeAccents($customer->name .' '.$customer->billing_name .' '. $customer->vat)) }}">
                                <td>
                                    {{ Form::checkbox('customer_id[]', $customer->id, $customer->ignore_mass_billing ? false : true, [$customer->ignore_mass_billing ? 'disabled' : '']) }}
                                </td>
                                <td>
                                    {{ @$customer->name }}
                                    @if(empty($customer->vat) || $customer->vat == '999999990')
                                    <br/><small><i class="fas fa-exclamation-triangle"></i> Sem NIF ou consumidor final</small>
                                    @endif
                                </td>
                                <td class="text-right">{{ money($price, $currency) }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
                @if($countEmptyVat)
                <p class="text-red"><i class="fas fa-info-circle"></i> {{ $countEmptyVat }} clientes ignorados. Sem NIF ou consumidor final.</p>
                @endif
            </div>
        </div>
    </div>

</div>
<div class="modal-footer">
    <div class="pull-right">
        <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
        @if(hasModule('invoices-advanced'))
        <button type="submit" class="btn btn-primary"
                data-loading-text="<i class='fas fa-spin fa-circle-notch'></i> A emitir faturas...">Emitir Faturas
        </button>
        @endif
    </div>
</div>
{{ Form::close() }}
<script>
    $('.datepicker').datepicker(Init.datepicker());
    $('.select2').select2(Init.select2());

    $('.customers-zone .row-all').on('change', function (){
        if($(this).is(':checked')) {
            $('[name="customer_id[]"]').prop('checked', true)
        } else {
            $('[name="customer_id[]"]').prop('checked', false)
        }

    })

    $('[name="send_email"]').on('change', function (){
        if($(this).is(':checked')) {
            $('[name="attachments[]"]').prop('disabled', false)
        } else {
            $('[name="attachments[]"]').prop('disabled', true)
        }
    })

    $('.customers-zone [name=search]').on('keyup', function (){
        var value = $(this).val().toLowerCase();
        var regex = new RegExp('\\b\\w*' + value + '\\w*\\b');
        $('.customers-zone [data-filter-text]').hide().filter(function () {
            return regex.test($(this).data('filter-text'));
        }).show();
    })

    $('.modal form').on('submit', function(e) {
        e.preventDefault();

        var action   = $(this).attr('action');
        var formData = $(this).serialize();
        var $btn     = $(this).find('button[type=submit]')

        $.post(action, formData, function(data) {

            if(data.result) {
                Growl.success(data.feedback);
                $('.customers-zone').html(data.html);
                $btn.hide()
                oTable.draw();
            } else {
                Growl.error(data.feedback)
            }

        }).fail(function (){
            Growl.error500()
        }).always(function(){
            $btn.button('reset');
        })
    })
</script>