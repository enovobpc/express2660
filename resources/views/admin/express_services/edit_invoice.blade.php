{{ Form::open(['route' => ['admin.express-services.invoice.create'], 'method' => 'post', 'class' => 'form-invoice']) }}
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal">
        <span class="fs-15" aria-hidden="true"><i class="fas fa-times"></i></span>
        <span class="sr-only">Fechar</span>
    </button>
    <h4 class="modal-title">Faturar serviços expresso</h4>
</div>
                ...
                @foreach($expressServices as $service)
                    ...
                    <tr>
                        <td class="text-center">
                            {{ Setting::get('invoice_item_express_service_ref') }}
                            {{ Form::hidden('ids[]', $service->id) }}
                        </td>
                        <td>{{ Form::text('description[]', '[' . $service->date . '] ' . $service->title, ['class' => 'form-control input-sm', 'required']) }}</td>
                        <td>{{ Form::select('tax[]', $taxes, null, ['class' => 'form-control input-sm', 'required']) }}</td>
                        <td>
                            <div class="input-group input-group-sm">
                                {{ Form::text('price[]', number($service->total_price), ['class' => 'form-control text-blue bold', 'required']) }}
