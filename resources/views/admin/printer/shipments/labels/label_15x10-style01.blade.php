        ...
        <div class="adhesive-block" style="width: 50mm; height: 12mm; text-align: left">
            @if($useAgenciesLogo)
                @if($shipment->recipientAgency->filepath_black && File::exists($shipment->recipientAgency->filepath_black))
                    <img src="{{ asset(@$shipment->recipientAgency->filehost . $shipment->recipientAgency->filepath_black) }}" style="height: 60px; max-width: 51mm; margin-top: 2mm" class="margin-left"/>
                @elseif($shipment->recipientAgency->filepath && File::exists($shipment->recipientAgency->filepath))
                    <img src="{{ asset(@$shipment->recipientAgency->filehost . $shipment->recipientAgency->filepath) }}" style="height: 60px; max-width: 51mm; margin-top: 2mm" class="margin-left"/>
                @else
           ...
           {{-- <div style="width: 1.1cm; height: 0.9cm; margin-right: 0mm; padding: 17px 5px 0 5px; font-weight: bold; font-size: 16px; border: 1px solid #000; text-align: center; float: right; color: #000">
            ...
            <div style="width: 2.5cm; height: 0.9cm; margin-right: 1mm; padding-top: 1px; line-height: 1.3; font-size: 9pt; border: 1px solid #fff; text-align: right; float: right; color: #000">
                ...
                @if(($shipment->recipient_country == 'pt' || $shipment->recipient_country == 'es') && @$shipment->route->code)
                    {{ @$shipment->route->code }} ({{ $shipment->senderAgency->code }})
                @else
...
@if(config('app.source') == 'activos24')
    <div style="text-align: center">
        <barcode code="ACT{{ $shipment->tracking_code }}" type="C128A" size="1.2" height="1" style="margin-left: -2mm; margin-top: 10px"/><br/>
        <small style="font-size: 10px">*ACT{{ $shipment->tracking_code }}*</small>
    </div>
@else
    <div style="margin-left:-3.85mm">
        <barcode code="{{ $shipment->tracking_code . str_pad($shipment->volumes, 3, '0', STR_PAD_LEFT) . str_pad($count, 3, '0', STR_PAD_LEFT) }}" type="C128A" size="1.075" height="1.4"/>
    </div>
@endif
