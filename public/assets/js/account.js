...
$(document).ready(function(){
    ...
    $.extend(true, $.fn.dataTable.defaults, {
        ...
        oLanguage: datatableLocales()

    });
});

function datatableLocales() {
...
$(document).ajaxError(function( event, jqxhr, settings, exception ) {
    ...
    if($('.modal.in .modal-content form').length == 0 && ($('#modal-remote-xs').hasClass('in') || $('#modal-remote-lg').hasClass('in') || $('#modal-remote-lg').hasClass('in') || $('#modal-remote-xl').hasClass('in'))) {
...
$(document).on('change', '[name=select-all]', function(){
    ...
    $('.row-select:first-child').trigger('change');
})
 ...
 */
$('body').on('loaded.bs.modal', '#modal-remote-xs, #modal-remote, #modal-remote-lg, #modal-remote-xl', function () {
    $('[data-toggle="tooltip"]').tooltip();
});
 ...
 * reset modal remote
 */
$('body').on('hidden.bs.modal', '#modal-remote, #modal-remote-xs, #modal-remote-lg, #modal-remote-xl', function () {

    if($(this).attr('id') == 'modal-remote-xl') {
        $(this).find('.modal-dialog').removeClass('modal-sm')
                                    .removeClass('modal-lg')
                                    .addClass('modal-xl');
    } else if($(this).attr('id') == 'modal-remote-xs') {
        $(this).find('.modal-dialog').removeClass('modal-sm')
                            .removeClass('modal-lg')
                            .addClass('modal-xs');
...
/**
 * Submit form by ajax - OLD METHOD
 *
 * @param {type} param1
 * @param {type} param2
 */
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.result) {
                ...
                $('input[type="checkbox"], input[type="radio"]').prop('checked', true)
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result) {
                ...
                if (data.dtbs) {
                    data.dtbs.forEach(function (value) {
                        try {
                            window[value].draw();
                        } catch (e) {}
                    });
