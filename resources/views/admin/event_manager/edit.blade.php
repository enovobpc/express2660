    ...
    <div class="row row-5">
        ...
        <div class="col-sm-3"  style="border-right: 1px solid #ccc;">
            <div class="form-group is-required">
                ...
                {{ Form::text('name', $event->name ?? '', ['class' => 'form-control uppercase', 'required']) }}
            </div>

            <div class="form-group">
                {{ Form::label('customer_id', 'Cliente', ['class' => 'control-label']) }}
                {!! Form::select('customer_id', [$event->customer_id => @$event->customer->code.' - '.@$event->customer->name] ?? [], null, ['class' => 'form-control select2', 'required']) !!}
            </div>

            <div class="form-group">
                {{ Form::label('type', 'Tipo') }}
                {{ Form::select('type', ['' => ''] + trans('admin/event-manager.types'), null, ['class' => 'form-control select2']) }}
            </div>
            <div class="form-group input-hours p-r-0">
                ...
                <div class="form-group form-group-sm m-b-5 col-sm-12">
                    ...
                    <div class="col-sm-7 p-l-0">
                        {{ Form::text('start_date', empty($event->date) ? date('Y-m-d') : null, ['class' => 'form-control datepicker']) }}
                    </div>
                ...
                <div class="form-group form-group-sm m-b-15 col-sm-12">
                    ...
                    <div class="col-sm-7 p-l-0">
                        {{ Form::text('end_date', empty($event->end_date) ? date('Y-m-d') : @$event->end_date->format('Y-m-d'), ['class' => 'form-control datepicker']) }}
                    </div>
                    <div class="col-sm-4 p-r-0 p-l-5" style="margin-left: -27px;">
                        {{ Form::select('end_hour', ['' => '--:--'] + $hours, empty($event->end_date) ? null : @$event->end_date->format('H:i'), ['class' => 'form-control select2']) }}
                    </div>
                </div>
            </div>
            <div class="form-group m-b-0">
                {{ Form::label('observation', 'Observações') }}
                {{ Form::textArea('observation', $event->observation ?? '', ['class' => 'form-control', 'rows' => 2]) }}
            </div>  
        </div>
        <div class="col-sm-9">
            <div class="col-sm-12">
                <div class="row row-5">
                    ...
                    <div class="col-sm-7">
                        <div class="form-group m-0">
                            ...
                            {{ Form::text('add_product[]', null, ['class' => 'form-control m-0 ' . (hasModule('logistic') ? 'search-sku' : '')]) }}
                    ...
                    <div class="col-sm-1">
                        <div class="form-group m-0">
                            ...
                            {{ Form::number('add_qty', null, ['class' => 'form-control number', 'min' => '0']) }}
                        </div>
                    </div>
                    <div class="col-sm-2">
            ...
            <div id="table-products"  class="col-sm-12">
                @include('admin.event_manager.partials.product_table')
            </div>
...
<script>
    ...
    function getEventId() {
        ...
        if (typeof eventId === 'undefined' || eventId == null || eventId == '') {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('admin.event-manager.store') }}',
                data: data,
