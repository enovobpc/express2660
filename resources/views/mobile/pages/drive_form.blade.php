...
<div class="window window-settings">
    ...
    overflow-y: scroll;
    overflow-x: hidden;">
        {{ Form::open(['route' => ['mobile.drive.store'], 'class' => 'ajax-form']) }}
        <h4 class="section-title">Registo de Condução</h4>
        @if(0)
            <h1 class="text-center" style="font-size: 50px">
                <small style="font-weight: 300;font-size: 28px">Em condução à</small><br/>
                00:00:00
            </h1>
            <div class="form-group is-required">
                {{ Form::label('kms', 'KM Finais') }}
                {{ Form::number('kms', null, ['class' => 'form-control select2', 'required']) }}
            </div>
            <div style="text-align: center; margin-top: 10px">
                <button class="btn-default">Parar/Terminar Condução</button>
            </div>
        @else
        <div class="form-group is-required">
            {{ Form::label('vehicle_id', 'Viatura') }}
            {{ Form::select('vehicle_id', ['' => ''] + $vehicles, @Auth::user()->vehicle_data->id, ['class' => 'form-control select2', 'required']) }}
        </div>
        <div class="form-group is-required">
            {{ Form::label('kms', 'KM Iniciais') }}
            {{ Form::number('kms', null, ['class' => 'form-control select2', 'required']) }}
        </div>
        <div style="text-align: center; margin-top: 10px">
            <button class="btn-default">Iniciar Condução</button>
        </div>
        @endif
        {{ Form::close() }}
    </div>
