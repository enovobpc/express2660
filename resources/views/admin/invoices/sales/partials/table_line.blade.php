@php
    $stockTotal     = @$billingProduct->stock_total + $qty;
    $stockTotalAttr = @$billingProductId ? 'data-stock-total="'. $stockTotal .'"' : null;
@endphp
...
<tr style="display: {{ ($i >= $rowsVisible && !empty($billing->lines)) || (empty($billing->lines) && $i > 0) ? 'none' : '' }}">
    <td style="vertical-align: top">
        ...
        <div class="input-group input-group-sm input-description w-100" style="position: relative">
            ...
            <i class="fas fa-plus-square btn-lineobs" data-toggle="tooltip" title="Adicionar detalhes"></i>
        </div>
        <div class="lineobs" style="{{ $lineObs ? '' : 'display:none' }}">
            {{ Form::textarea('line['.$key.'][obs]', @$lineObs, ['class' => 'form-control', 'rows' => '3', 'placeholder' => 'Detalhes ou informação complementar...']) }}
        </div>
    </td>
    <td style="vertical-align: top">
        {{ Form::text('line['.$key.'][qty]', $qty, ['class' => 'form-control input-sm text-center input-qty nospace decimal', $stockTotalAttr]) }}
    </td>
    <td style="vertical-align: top">
