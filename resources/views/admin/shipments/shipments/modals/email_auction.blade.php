@if(hasModule('auctions'))
{{ Form::open(['route' => ['admin.shipments.email.submit', 0, 'auction'], 'class' => 'form-ajax', 'method' => 'post']) }}
<div class="modal-header">
    <button type="button" class="close pull-right" data-dismiss="modal">
        <span class="fs-15" aria-hidden="true"><i class="fas fa-times"></i></span>
        <span class="sr-only">Fechar</span>
    </button>
    <h4 class="modal-title">Anunciar/leiloar carga</h4>
</div>
<div class="modal-body">
    <div class="row row-5">
        <div class="col-sm-8">
            <div class="form-group">
                {{ Form::label('mailing_list', 'Enviar para o grupo de e-mails') }}
                {{ Form::select('mailing_list', ['' => ''] + $mailingLists, null, ['class' => 'form-control select2', 'required']) }}
            </div>
        </div>
        <div class="col-sm-4">
            <div class="form-group">
                {{ Form::label('language', 'Língua e-mail') }}
                {{ Form::select('language', trans('locales'), null, ['class' => 'form-control select2']) }}
            </div>
        </div>
    </div>
</div>
<div class="modal-footer">
    <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
    <button type="submit" class="btn btn-primary" data-loading-text="A notificar...">Notificar Leilão</button>
</div>
{{ Form::hidden('ids', @$ids)}}
{{ Form::close() }}
<script>
    $('.modal .select2').select2(Init.select2());
</script>

@else
<div class="modal-header">
    <button type="button" class="close pull-right" data-dismiss="modal">
        <span class="fs-15" aria-hidden="true"><i class="fas fa-times"></i></span>
        <span class="sr-only">Fechar</span>
    </button>
    <h4 class="modal-title">Anunciar/leiloar carga</h4>
</div>
<div class="modal-body">
    <div class="row row-10">
        <div class="col-sm-3">
            <img class="w-100 m-t-20" src="data:image/svg+xml;utf8;base64,">
        </div>
        <div class="col-sm-9">
            <h4>Módulo extra não incluído.</h4>
            <p class="text-muted">
                <b>Esta funcionalidade não faz parte do sistema base e pode ser adquirido adicionalmente.</b>
            </p>
            <p>
                Promova a venda de carga e leilão da mesma a uma lista com vários destinatários/subcontratados e aguarde a resposta dos mesmos ao leilão.
                Pode depois visualizar e consultar as respostas no pedido de carga e atribuir a carga ao melhor subcontratado.
            </p>
            <a href="mailto:geral@enovo.pt?subject=Pedido de informação sobre módulo Leilão de cargas&amp;body=Pretendia conhecer mais detalhes e valor para o módulo de anuncio/leilão de carga." class="btn btn-xs btn-primary">
                <i class="fas fa-envelope"></i> Pedir informações
            </a>
        </div>
    </div>
</div>
<div class="modal-footer">
    <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
</div>
@endif
