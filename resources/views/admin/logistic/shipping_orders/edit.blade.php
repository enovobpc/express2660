<?php
$disabled = $shippingOrder->exists ? '' : 'disabled';
$customerDisabled =  @$shippingOrder->lines->count() > 0 ? 'disabled' : '';
$hash = str_random();
?>
...
<div class="modal-body modal-{{ $hash }}">
    <div class="row row-5">
        <div class="col-sm-5">
            <div class="form-group is-required">
                ...
                {{ Form::select('customer_id', @$shippingOrder->customer->exists ? [$shippingOrder->customer->id => $shippingOrder->customer->name] : ['' => ''], null, ['class' => 'form-control', 'required', $customerDisabled]) }}
            </div>
        </div>
        <div class="col-sm-3">
            <div class="form-group is-required">
        ...
        <div class="col-sm-2">
            <div class="form-group">
                ...
                {{ Form::select('status_id', ['' => ''] + $status, @$shippingOrder->exists ? null : \App\Models\Logistic\ShippingOrderStatus::STATUS_PENDING, ['class' => 'form-control select2', 'required']) }}
            </div>
        </div>
        <div class="col-sm-2">
            <div class="form-group is-required">
                ...
                <div class="input-group">
                    ...
                    {{ Form::text('date', $shippingOrder->exists ? null : date('Y-m-d'), ['class' => 'form-control datepicker', 'required']) }}
                </div>
            </div>
        </div>
    </div>
    <h4 class="m-b-10 m-t-0 bold"><i class="fas fa-boxes"></i> @trans('Artigos do pedido')</h4>
    <div class="row row-5">
    ...
    margin: 0 5px 0 5px;">
            <div class="col-sm-12">
                <div class="row row-5">
                    <div class="col-sm-7" style="width: 53%">
            ...
            <div class="col-sm-12 table-products">
                @include('admin.logistic.shipping_orders.partials.product_table')
            </div>
...
<div class="modal-footer">
    <div class="extra-options">
        <div class="checkbox">
            <label>
                {{ Form::checkbox('create_shipment', 1) }}
                @if(@$shippingOrder->shipment->id)
                    @trans('Ao gravar, editar expedição')
                @else
                    @trans('Ao gravar, criar expedição')
                @endif
            </label>
        </div>
    </div>
...
{{ Form::hidden('id', $shippingOrder->id) }}
{{ Form::close() }}
...
<script>
    ...
    $(MODAL + " select[name=customer_id]").select2({
        ajax: {
            url: "{{ route('admin.logistic.shipping-orders.search.customer') }}",
            dataType: 'json',
            method: 'post',
            delay: 250,
    ...
    $(MODAL + ' .search-product').select2({
        ajax: {
            url: "{{ route('admin.logistic.shipping-orders.search.product') }}",
            dataType: 'json',
            method: 'post',
            delay: 250,
            data: function (params) {
    ...
    $('[name=add_location]').on('change', function(){
        ...
        if(match) {
            ...
            maxUnities = $(MODAL + ' .search-product option:selected').data('stock');
        }

        $('[name=add_qty]').prop('placeholder', 'Max ' + maxUnities);
        $('[name=add_qty]').prop('data-max', maxUnities);
    });

    $('.btn-add-product').on('click', function(e) {
        e.preventDefault();

        var shippingOrder = $('.modal [name="id"]').val();
        ...
        var locationId = $(MODAL + ' [name="add_location"]').val();
        var qty        = parseInt($(MODAL + ' [name="add_qty"]').val());
        var maxQty     = parseInt($(MODAL + ' [name="add_qty"]').data('max'));
        var $btn       = $(this);
        ...
        if(product == '') {
            ...
            $btn.button('loading');
            $.post('{{ route('admin.logistic.shipping-orders.product.add') }}', data, function(data){

                if(data.result) {
                    $(MODAL).closest('form').find('[name="id"]').val(data.id);
                    $(MODAL + ' .table-products').html(data.html)
                    $(MODAL + ' [name="add_product"], [name="add_location"], [name="add_qty"]').val('');
                    $(MODAL + ' [name=add_qty]').prop('placeholder', '');
                    $(MODAL + ' [name=add_location] option, [name="add_product"] option').remove()
