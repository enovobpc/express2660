$(document).ready(function() {
    ...
    $('.btn-receive-call').on('click', function () {
        var $popup = $(this).closest('.chat-popup');
        $popup.addClass('expanded');
        $popup.find('.step2').slideDown('fast');
        $popup.find('.step1').slideUp('fast');

        var originalTitle = $popup.find('h4').html();
        $popup.find('h4').html($popup.find('h4').data('alternate-title'));
        $popup.find('h4').data('alternate-title', originalTitle);
    });
 ...
 * @param {type} param2
 */
$('form.ajax-webform').on('submit', function (e) {
    e.preventDefault();

    var $form = $(this);
    var $submitBtn = $form.find('button[type=submit]');
    $submitBtn.button('loading');
    $form.find('.form-loading').html('<div class="pull-left lds-ellipsis"><div></div><div></div><div></div><div></div></div>')
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (data.result) {
                ...
                $form.closest('.chat-content').find('form').hide();
                $form.closest('.chat-content').find('.form-feedback').show()
            ...
            if(data.feedback) {
                $form.find('.form-feedback').addClass(alertType).html(feedbackText).show()
            }
       ...
       /* swal({
        ...
        $form.find('.form-feedback').addClass('error').html('Não foi possível processar o seu pedido porque ocorreu um erro interno de servidor.')

    }).always(function () {
        $form.find('.form-loading').html('');
        $submitBtn.button('reset');
    });
});