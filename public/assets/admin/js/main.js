...
var EU_COUNTRIES = ["be","bg","cz","dk","de","ee","ie","el","es","fr","hr","it","cy","lv",
...
$(document).ready(function(){
  ...
  /*  if(DATATABLE_SEARCH_ON_ENTER == 1) {
    ...
    $.extend(true, $.fn.dataTable.defaults, {
        ...
        lengthMenu: [[10, 25, 50, 75, 100, 150, 200, 250, 300, 400, 500], [10, 25, 50, 75, 100, 150, 200, 250, 300, 400, 500]],
        order: [[1, "desc"]],
        ...
        oLanguage: Init.datatableTranslations()
    });
     ...
     */
     $(document).on('change', '[name=select-all]', function(){
        var $targetTable = $(this).closest('table');
        if($(this).is(':checked')) {
            $($targetTable).find('.row-select').prop('checked', true);
        } else {
            $($targetTable).find('.row-select').prop('checked', false);
        }

         $($targetTable).find('.row-select:first').trigger('change');
     })
    ...
    $(document).on('change', '.row-select',function(){
        ...
        $targetTable.find('input[name=row-select]:checked').each(function(i, selected){
            ...
            rowSubtotal = parseFloat($tr.find('[data-subtotal]').data('subtotal'));
            rowSubtotal = isNaN(rowSubtotal) ? 0 : rowSubtotal;
            tableSubtotal+= rowSubtotal;
            

            rowVat      = parseFloat($tr.find('[data-vat]').data('vat'));
            rowVat      = isNaN(rowVat) ? 0 : rowVat;
            tableVat+= rowVat;
            ...
            rowTotalKg   = parseFloat($tr.find('[data-vol]').data('kg'));
            rowTotalKg   = isNaN(rowTotalKg) ? 0 : rowTotalKg;
            tableTotalKg+= rowTotalKg;

            rowTotalVols   = parseFloat($tr.find('[data-vol]').data('vol'));
            rowTotalVols   = isNaN(rowTotalVols) ? 0 : rowTotalVols;
            tableTotalVols+= rowTotalVols;

            rowTotalM3   = parseFloat($tr.find('[data-vol]').data('m3'));
            rowTotalM3   = isNaN(rowTotalM3) ? 0 : rowTotalM3;
            tableTotalM3+= rowTotalM3;

            rowTotalLdm   = parseFloat($tr.find('[data-vol]').data('ldm'));
            rowTotalLdm   = isNaN(rowTotalLdm) ? 0 : rowTotalLdm;
            tableTotalLdm+= rowTotalLdm;
        });

        $('.dt-sum-subtotal').html(tableSubtotal.toFixed(2));
        $('.dt-sum-vat').html(tableVat.toFixed(2));
        $('.dt-sum-total').html(tableTotal.toFixed(2));
        $('.dt-sum-kg').html(tableTotalKg.toFixed(2));
        $('.dt-sum-vol').html(tableTotalVols);
        $('.dt-sum-m3').html(tableTotalM3.toFixed(2));
        $('.dt-sum-ldm').html(tableTotalLdm.toFixed(2));
...
$(document).on('click', '[data-ajax-confirm], [data-toggle="ajax-confirm"]', function(e){
    ...
    var callback     = $this.data("confirm-callback");
...
$(document).on('submit', '[data-toggle="ajax-form"]',function(e){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result) {
                console.log($form.data('refresh-datatables'));
                if($form.data('refresh-datatables')) {
                    try {
                        if ($form.data('target-datatables')) {
                            window[$form.data('target-datatables')].draw();
                        } else {
                            oTable.draw();
                        }
                    } catch(e) {}
...
$(document).ajaxError(function( event, jqxhr, settings, exception ) {
    if($('.modal.in .modal-content form').length == 0 && ($('#modal-remote').hasClass('in') || $('#modal-remote-lg').hasClass('in') || $('#modal-remote-xlg').hasClass('in') || $('#modal-remote-xl').hasClass('in'))) {
        html = '<h4 class="modal-title text-center m-t-40 m-b-15 text-red">' +
            '<i class="fas fa-exclamation-circle fs-40"></i><br> Operação indisponível.' +
            '<br><span class="fw-400">Não foi possível abrir a janela devido a um problema de configuração. Tente novamente ou reporte a situação.</span>' +
            '</h4>' +
...
$('body').on('shown.bs.modal', '#modal-remote, #modal-remote-lg, #modal-remote-xlg, #modal-remote-xl, #modal-remote-xs', function () {
    $('.datepicker').datepicker(Init.datepicker());
    modalInstance = $(this);
});

//reset modal remote
$('body').on('hidden.bs.modal', '#modal-remote, #modal-remote-lg, #modal-remote-xlg, #modal-remote-xl, #modal-remote-xs', function (e) {
    ...
    if($(this).attr('id') == 'modal-remote-xl') {
        ...
        $(this).find('.modal-dialog').removeClass('modal-sm')
                                     ...
                                     .removeClass('modal-xlg')
                                     .addClass('modal-lg');
    }else if($(this).attr('id') == 'modal-remote-xlg') {
        $(this).find('.modal-dialog').removeClass('modal-sm')
                                     .removeClass('modal-xl')
                                     .addClass('modal-xlg');
...
$(document).on('click', '.kg-adicional', function () {
    var $target = $(this).find('i');
    var $row    = $(this).closest('tr');

    if($target.hasClass('fa-square')) { //ativa
        $target.removeClass('far fa-square');
        $target.addClass('fas fa-check-square');
        $row.addClass('rw-adicional')

        $row.find('.kg-adc-unity').show();
        $row.find('[name="is_adicional[]"]').val(1);
        $row.find('[name="adicional_unity[]"]').val(1);
    } else { //desativa
        $target.addClass('far fa-square');
        $target.removeClass('fas fa-check-square');
        $row.removeClass('rw-adicional')

        $row.find('.kg-adc-unity').hide();
        $row.find('[name="is_adicional[]"]').val(0);
        $row.find('[name="adicional_unity[]"]').val(1);
    }
})
...
function addLineAfter($target, $this){
    ...
    //disable kg adicional
    $trNew.removeClass('rw-adicional')
    $trNew.find('.kg-adc-unity').hide();
    $trNew.find('[name="is_adicional[]"]').val(0);
    $trNew.find('[name="adicional_unity[]"]').val('1');
    $trNew.find('.kg-adicional i').addClass('far fa-square');
    $trNew.find('.kg-adicional i').removeClass('fas fa-check-square');
...
$('input[data-max-size]').bind('change', function(){
    ...
    if($(this).val() != '') {
        ...
        if(fileSize > maxBytes) {
            ...
            */
            Growl.error('O tamanho do ficheiro excede o máximo permitido de '+ max + unity)
        }
    }
})
...
$('input[data-file-format]').bind('change', function(){
    ...
    if($(this).val() != '') {
        ...
        if($.inArray(extension,acceptedExtensions) < 0) {
            ...
            Growl.error('A extenção do ficheiro selecionado não é permitida.')
            /*swal({
                title: "",
...
$(document).on('click', '.form-fast-search [data-toggle="tab"]',function(e){
    e.preventDefault();
    var target = $(this).data('starget');
    $('.form-fast-search [name="target"]').val(target)
});