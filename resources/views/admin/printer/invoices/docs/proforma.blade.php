...
<div style="width: 100%; height: 14mm; text-align: right; position: absolute; top: -10px; right: 10px;">
    ...
    <h4 style="margin: 3px 0 3px; font-size: 18px; font-weight: bold">Fatura Proforma N.º {{ strtoupper($invoice->doc_series) }} {{ $invoice->doc_id }}</h4>
        ...
        <div class="guide-row" style="padding-top: 0mm;">
            <div class="fs-7px lh-1-2" style="width: 50%; height: 14mm; float: left;">
                <p style="margin-top: 10px; margin-bottom: 4px; line-height: 14px; font-size: 11px;">
                    <b style="font-weight: bold; font-size: 13px; margin-bottom: 4px">{{ $agency->company }}</b><br>
                </p>
                <p style="line-height: 14px; font-size: 11px;">
                    {{ $agency->address }}
                    <br/>
                    {{ $agency->zip_code }} {{ $agency->city }}
                    <br/>
                    {{ trans('country.' . $agency->country) }}
                </p>
                <p style="font-size: 11px; line-height: 16px">
                    <strong style="font-weight: bold">Contribuinte:</strong> {{ $agency->vat }}<br/>
                    @if(Setting::get('company_phone'))
                        <strong style="font-weight: bold">Telefone:</strong> {{ $agency->phone ?: Setting::get('company_phone') }}<br/>
                    @endif
                    @if(Setting::get('company_email'))
                        <strong style="font-weight: bold">E-mail:</strong> {{ $agency->email ?: Setting::get('company_email') }}<br/>
                    @endif
                    @if(Setting::get('company_website'))
                        <strong style="font-weight: bold">Website:</strong> {{ $agency->web ?: Setting::get('company_website') }}<br/>
                    @endif

                    @if(Setting::get('company_capital'))
                        <strong>Capital Social:</strong> {{ $agency->capital ?: Setting::get('company_capital') }}<br/>
                    @endif
                    <br/>
        ...
        <div class="guide-row">
            <div class="lh-1-2" style="width: 430px; height: 24mm; margin-bottom: -10px; float: left;">
                ...
                <table style="width: 100%; font-size: 11px">
                    ...
                    @if(!empty(@$taxesExempt))
                        @foreach($taxesExempt as $taxReason => $taxItems)
                            <?php

                                $taxIncidence = $taxItems->sum('subtotal');
                                $taxTotal     = $taxIncidence;
                            ?>
                            <tr>
