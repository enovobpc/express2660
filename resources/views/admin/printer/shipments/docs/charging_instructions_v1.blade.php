    ...
    <div class="guide-content">
        <div class="guide-row" style="padding-top: 5mm;">
            ...
            <div class="guide-block-right" style="width: 50%; border: 1px solid #000; border-radius: 10px; padding: 10px; float: left; height: 35mm">
                ...
                @endif
                @if (@$shipment->provider->phone)
                <br/>
                Phone: {{ @$shipment->provider->phone }}<br/>
                @endif
            </div>
        </div>

        <div class="guide-row" style="margin-top: 10mm">
            @if(Setting::get('charging_instructions_presentation_' . $locale))
            <div class="lh-1-2 fs-12">
            ...
            <div class="guide-row">
                <div style="width: 100%; float: left; border: 1px solid #333; border-radius: 4px; background: #fff">
                    ...
                    <div style="width: 38%; float: left;padding: 2px 10px">
                        <div style="border-bottom: 1px solid #777;">
                            {{ transLocale('admin/global.word.from', $locale) }}: <b style="font-weight: bold; text-transform: uppercase">{{ strtoupper($shipment->sender_country) . ' ' . $shipment->sender_zip_code . ' ' . $shipment->sender_city }}</b>
                        </div>
                        <div style="border-bottom: 1px solid #777;">
                        ...
                        @if ($shipment->operator->professional_mobile ?? false)
                        <div style="border-top: 1px solid #777;">
                            Phone: {{ $shipment->operator->professional_mobile }}
                        </div>
                        @endif
                    </div>
                    <div style="width: 38%; float: left;padding: 2px 10px">
                        <div style="border-bottom: 1px solid #777;">
                            {{ transLocale('admin/global.word.to', $locale) }}: <b style="font-weight: bold; text-transform: uppercase">{{ strtoupper($shipment->recipient_country) . ' ' . $shipment->recipient_zip_code . ' ' .$shipment->recipient_city }}</b>
                        </div>
                        <div style="border-bottom: 1px solid #777;">
                            {{ transLocale('admin/global.word.shipped_by', $locale) }}: {{ Auth::user()->name }}<br/>
                        </div>
                        <div>
                            {{ transLocale('admin/global.word.total_price', $locale) }}: {{ money($shipment->cost_billing_subtotal, 'EUR') }}
                        </div>
        ...
        <h4 style="font-weight: bold; margin-top: 20px">{{ strtoupper(transLocale('admin/global.word.goods_to_transport', $locale)) }}</h4>
        <div class="guide-row">
            <div style="width: 100%; float: left; border: 1px solid #333; border-radius: 4px; background: #fff; height: {{ $shipment->addresses ? '190px' : '' }}">
                    ...
                    @if(!$shipment->pack_dimensions->isEmpty())
                        @foreach($shipment->pack_dimensions as $pack)
                            <tr>
                                ...
                                <td class="text-center">{{ strtoupper($shipment->sender_country) . ' ' . $shipment->sender_zip_code . ' ' . $shipment->sender_city }}</td>
                                <td class="text-center">{{ strtoupper($shipment->recipient_country) . ' ' . $shipment->recipient_zip_code . ' ' . $shipment->recipient_city }}</td>
                            </tr>
                        @endforeach
                    @else
                        <tr>
                            <td></td>
                            <td class="text-center" style="padding: 5px 0">{{ money($shipment->volumes) }}</td>
        ...
        <div class="guide-row m-b-5" style="border: 2px solid #333; border-top: none; border-radius: 0 0 4px 4px; background: #fff;">

            <div style="width: 49.8%;float: left; border-right: 1px solid #ccc;">
                <div style="float: left;">
                    <div style="border-bottom: 1px solid #ccc; background: #f2f2f2">
                        <div style="text-align: right; float: right; width: 63%; padding: 2px 5px 0 0;">
                            {{ transLocale('admin/global.word.datetime', $locale) }}: {{ $shipment->date }} {{ $shipment->start_hour }}
                        </div>
                        <div style="font-size: 13px; float: left; font-weight: bold;  width: 30%; padding: 0 0 0 5px">{{ strtoupper(transLocale('admin/global.word.pickup', $locale)) }}</div>
                    </div>
                    ...
                    <div style="padding: 2px 10px; border-bottom: none">
            ...
            <div style="width: 50%;float: left;">
                <div style="float: left;">
                    <div style="border-bottom: 1px solid #ccc; background: #f2f2f2">
                        <div style="text-align: right; float: right; width: 63%; padding: 2px 5px 0 0;">
                            {{ transLocale('admin/global.word.datetime', $locale) }}: {{ @$shipment->delivery_date ? $shipment->delivery_date->format('Y-m-d H:i') : 'N/A' }}
                        </div>
                        <div style="font-size: 13px; float: left; font-weight: bold;  width: 30%; padding: 0 0 0 5px">{{ strtoupper(transLocale('admin/global.word.delivery', $locale)) }}</div>
                    </div>
                    <div style="padding: 5px 10px; border-bottom: 1px solid #ccc">
                        <p class="lh-1-3 m-0" style="height: 21mm">
            ...
            <div class="guide-row m-b-5" style="border: 2px solid #333; border-top: none; border-radius: 0 0 4px 4px; background: #fff;">
                ...
                <div style="width: 49.8%;float: left; border-right: 1px solid #ccc;">
                    <div style="float: left;">
                        ...
                        <div style="padding: 5px 10px; border-bottom: 1px solid #ccc">
                            <p class="lh-1-3 m-0" style="height: 18mm">
                                {{ @$address->sender_name }}<br>
                                {{ @$address->sender_address }}<br/>
                                {{ @$address->sender_zip_code }} {{ @$address->sender_city }}
                    ...
                    <div style="float: left;">
                        ...
                        <div style="padding: 5px 10px; border-bottom: 1px solid #ccc">
                            <p class="lh-1-3 m-0" style="height: 18mm">
                                ...
                                {{ @$address->recipient_zip_code }} {{ @$address->recipient_city }}
        ...
        {{-- FORÇA A MUDAR DE LINHA --}}
        @if($shipment->addresses)
            <div style="height: 30px"></div>
        @elseif(Setting::get('charging_instructions_presentation_' . $locale))
            <div style="height: 120px"></div>
        @endif
        <h4 style="font-weight: bold; margin-top: 20px">{{ transLocale('admin/global.word.service_billing', $locale) }}</h4>
            ...
            <div style="width: 100%; float: left; border: 1px solid #333; border-radius: 4px; background: #fff;">
                <table class="table table-bordered fs-10" style="margin: 0">
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ money($shipment->cost_shipping_price) }}</td>
                    </tr>
                    @foreach($shipment->expenses as $expense)
                        @if($expense->pivot->cost_subtotal > 0.00)
                        <tr>
                            <td>{{ $expense->name }}</td>
                            <td class="text-center">{{ money($expense->pivot->qty) }}</td>
                            <td class="text-center">{{ money($expense->pivot->cost_subtotal) }}</td>
                        </tr>
                        @endif
                    @endforeach
                </table>
        ...
        <div class="guide-row">
            <div style="width: 100%; float: left; border: 1px solid #333; border-radius: 4px; background: #fff;">
                <div style="width: 100%; float: left;padding: 2px 10px">
                    <p style="font-weight: bold; margin: 0">{{ transLocale('admin/global.word.payment_method', $locale) }}: {{ @$shipment->provider->payment_method ? ($locale == 'pt' ? $shipment->provider->paymentCondition->name : trans('admin/billing.payment-conditions.' . $shipment->provider->paymentCondition->code, [] , 'messages', $locale)) : '30 dias' }}</p>
                </div>
                ...
                <div style="width: 45%; float: left;padding: 2px 10px">
                    <p style="font-weight: bold; margin: 0">{{ transLocale('admin/global.word.billing_address', $locale) }}</p>
                    <p style="margin: 0">
                        {{ @$shipment->agency->company }}<br>
                        {{ @$shipment->agency->address }}<br/>
                        {{ @$shipment->agency->zip_code }} {{ @$shipment->agency->city }}
                        - {{ trans('country.' . @$shipment->agency->country) }}<br/>
                        {{ @$shipment->agency->billing_email }}
                    </p>
                </div>
            </div>
        </div>
...
<div class="font-size-6pt" style="padding-left: 10mm; padding-right: 10mm;">
    ...
    <div class="pull-left" style="width: 57%">Processed by {{ app_brand('docsignature') }}</div>
