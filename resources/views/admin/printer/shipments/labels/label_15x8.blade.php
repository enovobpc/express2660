<div class="adhesive-label" style="width: 80mm; height: 100mm">
    <div class="adhesive-row">
        <div class="adhesive-block" style="float: left; width: 56mm; height: 16mm; text-align: left">
            @if($useAgenciesLogo)
            ...
            @else
                @if(@$shipment->agency->filepath_black && File::exists($shipment->agency->filepath_black))
                    <img src="{{ asset($shipment->agency->filepath_black) }}" style="height: 40px;" class="m-t-20"/>
                @elseif($shipment->agency->filepath && File::exists($shipment->agency->filepath))
                    <img src="{{ asset($shipment->agency->filepath) }}" style="height: 40px;" class="m-t-20"/>
                @else
    ...
    <div style="height: 41mm; float: left;">
                ...
                <div class="adhesive-block">
                    ...
                    <div>
                        <div class="fs-12pt bold lh-1-2">
                            {{ str_limit(strtoupper($shipment->recipient_name), 27) }}<br>
     ...
     <div class="adhesive-row">
        <div class="adhesive-block" style="height: 5mm">
            @if($shipment->obs)
                <div class="fs-9pt m-t-3 p-b-6">{{ $shipment->obs }}</div>
            @endif
...
<div class="text-center">
    <div class="adhesive-row" style="margin-top: 10px; margin-bottom: 10px; padding: 2px 10px 2px 10px; background: #333; color: #fff; border-bottom: 0; width: 80mm">
        <div class="adhesive-block" style="width: 2.5cm; height: 6.1mm; float: left">
            <div class="fs-14pt bold text-left">
                {{ str_pad($count, 3, "0", STR_PAD_LEFT) }}/{{ str_pad($shipment->volumes, 3, "0", STR_PAD_LEFT) }}
            </div>
        </div>
        <div class="adhesive-block" style="width: 4.5cm; height: 6.1mm">
    ...
    <div style="display: inline-block; margin-top: 5px;">
        @if(strlen($shipment->recipientAgency->print_name) > 18)
            <barcode code="{{ $shipment->tracking_code . str_pad($shipment->volumes, 3, '0', STR_PAD_LEFT) . str_pad($count, 3, '0', STR_PAD_LEFT) }}" type="C128A" size="0.85" height="1.1"/>
        @else
            <barcode code="{{ $shipment->tracking_code . str_pad($shipment->volumes, 3, '0', STR_PAD_LEFT) . str_pad($count, 3, '0', STR_PAD_LEFT) }}" type="C128A" size="0.85" height="1.4"/>
        @endif
    </div>
