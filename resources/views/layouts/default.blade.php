...
<html lang="{{ Lang::locale() }}">
    <head>
      ...
      {{--{{ Html::style('/vendor/intl-tel-input/build/css/intlTelInput.min.css') }}--}}
        ...
        @if(config('app.source') == 'papiro')
            {{ Html::style('/assets/css/default_papiro.css') }}
        @endif
        ...
        @if($auth)
            <body class="account {{ app_skin() }}">
            @include('default.header')
        @else
            @if($LOGINMODE == 'SIMPLE')
            <body class="account login-simple {{ app_skin() }}">
            @elseif($LOGINMODE == 'STYLE01')
            <body class="account {{ app_skin() }}">
            @else
                @if(Route::currentRouteName() == 'tracking.index')
                <body class="account {{ app_skin() }}">
                @else
                <body class="account {{ app_skin() }}" style="background-image: url('{{ asset('assets/img/default/bg/' . str_pad (rand(1,4) , 2, $pad_string = "0", STR_PAD_LEFT)) }}.jpg');">
                @endif
            @endif
        @endif
        ...
        @if(config('app.source') == 'entregaki')
            <script>
                var MessageBirdChatWidgetSettings = {
                    widgetId: 'aeea2165-503c-4536-b019-bfa105e34925',
        ...
        <script>
            ...
            var SEARCH_ZIP_CODE         = {{ in_array(config('app.source'), ['baltrans', 'pontualhd']) ? 'true' : 'false' }};
            var ROUTE_SEARCH_ZIP_CODES  = "{{ route('account.shipments.zip-codes.search') }}";
        </script>
